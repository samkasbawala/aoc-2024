...
def solve(input_string: str) -> int:
    ...
    # Find anti nodes
    anti_nodes_coordinates = set(
        filter(lambda x: _in_bounds(x, area), _find_anti_nodes(coordinates))
    )
...
def _find_anti_nodes(
    ...
    for _, locations in coordinates.items():
        anti_nodes = anti_nodes.union(_process_signal(locations))

    return anti_nodes


def _process_signal(locations: set[tuple[int, int]]) -> set[tuple[int, int]]:
    ...
    for i in range(len(locations) - 1):
        for j in range(i + 1, len(locations)):
            ...
            anti_nodes.add((x1 - run, y1 - rise))
            anti_nodes.add((x2 + run, y2 + rise))
